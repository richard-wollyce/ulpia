...
fn index_md(title: &str) -> String {
    ...
    format!(
         ...
         **Search for:** `{lower}`, `agente {lower}`, `quem e {lower}`, `who is {lower}`, \
         `o que {lower} faz`, `what does {lower} do`, `operating instructions`, \
         `instrucoes de operacao`, `constituicao`, `constitution`, `ordem de leitura`, \
         `reading order`, `limites`, `limits`, `o que nunca faz`, `never does`, `escopo`, \
         `scope`, `papel`, `role`, `competencia`, `capability`\n\n\
